...
def receitas_contribuicoes():
    ...
    try:
        ...
        # ATUALIZAÇÃO: Agora retorna 6 valores incluindo comparativo_mensal
        dados_tabela, mes_referencia, dados_para_ia, dados_pdf, resumo_nougs, comparativo_mensal = gerar_relatorio_receitas_contribuicoes(
            df_completo, HIERARQUIA_RECEITAS, noug_selecionada
        )
...
@receita_bp.route('/receitas-amortizacao-emprestimo')
def receitas_amortizacao_emprestimo():
    """Relatório de receitas de amortização de empréstimos líquidas - RELATÓRIO DETALHADO"""
    try:
        inicio = time.time()
        df_completo = carregar_dataframe_receita()
        lista_nougs = sorted(df_completo['NOUG'].dropna().unique().tolist())
        ...
        fim = time.time()
        print(f"⏱️ Relatório de Receitas de Amortização de Empréstimos gerado em {fim - inicio:.2f} segundos")

        return render_template('relatorio_receitas_amortizacao_emprestimo.html',
                               dados_relatorio=dados_tabela,
                               mes_ref=mes_referencia,
                               dados_para_ia=dados_para_ia,
                               dados_pdf=dados_pdf,
        ...
        traceback.print_exc()
        return render_template('erro.html',
                             titulo="Erro no Relatório de Receitas de Amortização de Empréstimos",
                             mensagem=f"Erro ao gerar relatório: {str(e)}")

@receita_bp.route('/receitas-transferencia-capital')
def receitas_transferencia_capital():
    """Relatório de receitas de transferências de capital líquidas - RELATÓRIO DETALHADO"""
    try:
        inicio = time.time()
        df_completo = carregar_dataframe_receita()
        lista_nougs = sorted(df_completo['NOUG'].dropna().unique().tolist())
        ...
        fim = time.time()
        print(f"⏱️ Relatório de Receitas de Transferências de Capital gerado em {fim - inicio:.2f} segundos")

        return render_template('relatorio_receitas_transferencia_capital.html',
                               dados_relatorio=dados_tabela,
                               mes_ref=mes_referencia,
                               dados_para_ia=dados_para_ia,
                               dados_pdf=dados_pdf,
