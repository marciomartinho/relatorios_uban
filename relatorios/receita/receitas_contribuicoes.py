"""
Relatório: Receitas de Contribuições Líquidas Realizada
Analisa receitas das origens 12 e 72 com desdobramentos por alínea
REGRAS:
- ORIGEM = 12 e 72 (receitas de contribuições)
...
def gerar_relatorio_receitas_contribuicoes(df_completo, estrutura_hierarquica=None, noug_selecionada=None):
    """
    Gera relatório de receitas de contribuições líquidas com comparativo 2024 vs 2025
    
    REGRAS DE NEGÓCIO:
    - Filtra apenas ORIGEM = 12 e 72 (receitas de contribuições)
    ...
    # Filtra apenas origens de contribuições (12 e 72) para 2025 e 2024
    df_2025 = df_processar[
        (df_processar['COEXERCICIO'] == 2025) & 
        (df_processar['ORIGEM'].isin(['12', '72']))
    ]
    ...
    print(f"✅ Relatório de receitas de contribuições gerado: {len(dados_numericos)} linhas (espécies + alíneas + total)")
...
def _gerar_comparativo_mensal_acumulado(df_completo, motor, mes_referencia):
    ...
    """
    try:
        # Filtra apenas origens de contribuições (12 e 72)
        df_contribuicoes = df_completo[df_completo['ORIGEM'].isin(['12', '72'])].copy()
        
        if df_contribuicoes.empty or 'INMES' not in df_contribuicoes.columns:
            print("⚠️ Dados insuficientes para comparativo mensal")
            return []
        
        # Obtém meses disponíveis em 2025
        meses_2025 = sorted(df_contribuicoes[df_contribuicoes['COEXERCICIO'] == 2025]['INMES'].dropna().unique())
        meses_2024 = sorted(df_contribuicoes[df_contribuicoes['COEXERCICIO'] == 2024]['INMES'].dropna().unique())
        ...
        for mes_atual in range(1, max_mes):  # Para até max_mes-1
            ...
            df_2025_acum = df_contribuicoes[
                (df_contribuicoes['COEXERCICIO'] == 2025) &
                (df_contribuicoes['INMES'] <= mes_atual)
            ]
            ...
            df_2024_acum = df_contribuicoes[
                (df_contribuicoes['COEXERCICIO'] == 2024) &
                (df_contribuicoes['INMES'] <= mes_atual)
            ]
