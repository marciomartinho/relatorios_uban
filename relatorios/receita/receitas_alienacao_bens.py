"""
Relatório: Receita de Alienação de Bens Líquida Realizada
Analisa receitas da origem 22 com desdobramentos por alínea
REGRAS:
- ORIGEM = 22 (receitas de alienação de bens)
...
def gerar_relatorio_receitas_alienacao_bens(df_completo, estrutura_hierarquica=None, noug_selecionada=None):
    """
    Gera relatório de receitas de alienação de bens líquidas com comparativo 2024 vs 2025
    
    REGRAS DE NEGÓCIO:
    - Filtra apenas ORIGEM = 22 (receitas de alienação de bens)
    ...
    # Filtra apenas origem de alienação de bens (22) para 2025 e 2024
    df_2025 = df_processar[
        (df_processar['COEXERCICIO'] == 2025) & 
        (df_processar['ORIGEM'] == '22')
    ]
    ...
    print("🔍 Processando receitas de alienação de bens (ORIGEM 22)...")
    ...
    # Processa cada espécie encontrada (combinando 2024 e 2025)
    ...
    especies_disponiveis = sorted(especies_2025.union(especies_2024))
    
    print(f"📊 Espécies encontradas (combinadas): {especies_disponiveis}")
    
    for especie in especies_disponiveis:
        print(f"\n🏛️ Processando espécie: {especie}")
        
        # Busca nome da espécie (prioriza 2025, depois 2024)
    ...
    print(f"✅ Relatório de receitas de alienação de bens gerado: {len(dados_numericos)} linhas (espécies + alíneas + total)")
...
def _gerar_comparativo_mensal_acumulado(df_completo, motor, mes_referencia):
    ...
    """
    try:
        # Filtra apenas origem 22 (alienação de bens)
        df_alienacao = df_completo[df_completo['ORIGEM'] == '22'].copy()
        
        if df_alienacao.empty or 'INMES' not in df_alienacao.columns:
            print("⚠️ Dados insuficientes para comparativo mensal")
            return []
        
        # Obtém meses disponíveis em 2025
        meses_2025 = sorted(df_alienacao[df_alienacao['COEXERCICIO'] == 2025]['INMES'].dropna().unique())
        meses_2024 = sorted(df_alienacao[df_alienacao['COEXERCICIO'] == 2024]['INMES'].dropna().unique())
        ...
        for mes_atual in range(1, max_mes):  # Para até max_mes-1
            ...
            df_2025_acum = df_alienacao[
                (df_alienacao['COEXERCICIO'] == 2025) &
                (df_alienacao['INMES'] <= mes_atual)
            ]
            ...
            df_2024_acum = df_alienacao[
                (df_alienacao['COEXERCICIO'] == 2024) &
                (df_alienacao['INMES'] <= mes_atual)
            ]
