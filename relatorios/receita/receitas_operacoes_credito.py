"""
Relatório: Receita de Operações de Crédito Líquida Realizada
Analisa receitas da origem 21 com desdobramentos por alínea
REGRAS:
- ORIGEM = 21 (receitas de operações de crédito)
...
def gerar_grafico_receita_capital(df_completo, estrutura_hierarquica=None, noug_selecionada=None):
    """
    Gera relatório de receitas de operações de crédito líquidas com comparativo 2024 vs 2025
    
    REGRAS DE NEGÓCIO:
    - Filtra apenas ORIGEM = 21 (receitas de operações de crédito)
    ...
    print("🔍 Processando receitas de operações de crédito (ORIGEM 21)...")
    ...
    print(f"✅ Relatório de receitas de operações de crédito gerado: {len(dados_numericos)} linhas (espécies + alíneas + total)")
...
def _gerar_comparativo_mensal_acumulado(df_completo, motor, mes_referencia):
    ...
    """
    try:
        # Filtra apenas origem 21 (operações de crédito)
        df_operacoes = df_completo[df_completo['ORIGEM'] == '21'].copy()
        
        if df_operacoes.empty or 'INMES' not in df_operacoes.columns:
            print("⚠️ Dados insuficientes para comparativo mensal")
            return []
        
        # Obtém meses disponíveis em 2025
        meses_2025 = sorted(df_operacoes[df_operacoes['COEXERCICIO'] == 2025]['INMES'].dropna().unique())
        meses_2024 = sorted(df_operacoes[df_operacoes['COEXERCICIO'] == 2024]['INMES'].dropna().unique())
        ...
        for mes_atual in range(1, max_mes):  # Para até max_mes-1
            ...
            df_2025_acum = df_operacoes[
                (df_operacoes['COEXERCICIO'] == 2025) &
                (df_operacoes['INMES'] <= mes_atual)
            ]
            ...
            df_2024_acum = df_operacoes[
                (df_operacoes['COEXERCICIO'] == 2024) &
                (df_operacoes['INMES'] <= mes_atual)
            ]
