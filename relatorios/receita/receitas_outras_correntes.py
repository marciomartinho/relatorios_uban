...
def _gerar_comparativo_mensal_acumulado(df_completo, motor, mes_referencia):
    ...
    """
    try:
        # Filtra apenas origens de outras receitas correntes
        df_outras_correntes = df_completo[df_completo['ORIGEM'].isin(['79', '99'])].copy()
        
        if df_outras_correntes.empty or 'INMES' not in df_outras_correntes.columns:
            print("⚠️ Dados insuficientes para comparativo mensal")
            return []
        
        # Obtém meses disponíveis em 2025
        meses_2025 = sorted(df_outras_correntes[df_outras_correntes['COEXERCICIO'] == 2025]['INMES'].dropna().unique())
        meses_2024 = sorted(df_outras_correntes[df_outras_correntes['COEXERCICIO'] == 2024]['INMES'].dropna().unique())
        ...
        for mes_atual in range(1, max_mes):  # Para até max_mes-1
            ...
            df_2025_acum = df_outras_correntes[
                (df_outras_correntes['COEXERCICIO'] == 2025) &
                (df_outras_correntes['INMES'] <= mes_atual)
            ]
            ...
            df_2024_acum = df_outras_correntes[
                (df_outras_correntes['COEXERCICIO'] == 2024) &
                (df_outras_correntes['INMES'] <= mes_atual)
            ]
