"""
Relatório: Receitas Industriais Líquidas Realizada
Analisa receitas das origens 15 e 75 com desdobramentos por alínea
REGRAS:
- ORIGEM = 15 e 75 (receitas industriais)
...
def gerar_relatorio_receitas_industriais(df_completo, estrutura_hierarquica=None, noug_selecionada=None):
    """
    Gera relatório de receitas industriais líquidas com comparativo 2024 vs 2025
    
    REGRAS DE NEGÓCIO:
    - Filtra apenas ORIGEM = 15 e 75 (receitas industriais)
    ...
    # Filtra apenas origens industriais (15 e 75) para 2025 e 2024
    df_2025 = df_processar[
        (df_processar['COEXERCICIO'] == 2025) & 
        (df_processar['ORIGEM'].isin(['15', '75']))
    ]
    ...
    if 'RECEITA LIQUIDA' not in df_2025.columns:
        print("[AVISO] Coluna 'RECEITA LIQUIDA' não encontrada na planilha")
        return [], obter_mes_numero(df_processar), [], {}, [], []
    
    # Verifica se as colunas ALINEA e NOALINEA existem
    if 'ALINEA' not in df_2025.columns or 'NOALINEA' not in df_2025.columns:
        print("[AVISO] Colunas 'ALINEA' ou 'NOALINEA' não encontradas na planilha")
        return [], obter_mes_numero(df_processar), [], {}, [], []
    
    print("[INFO] Processando receitas industriais (ORIGEM 15 e 75)...")
    
    # Debug: Verificar estrutura dos dados
    print(f"[DADOS] Dados 2025 - Total de registros: {len(df_2025)}")
    print(f"[DADOS] Dados 2024 - Total de registros: {len(df_2024)}")
    
    if not df_2025.empty:
        print(f"[DADOS] Colunas disponíveis: {list(df_2025.columns)}")
        print(f"[DADOS] Espécies em 2025: {sorted(df_2025['ESPECIE'].dropna().unique())}")
        if 'ALINEA' in df_2025.columns:
            print(f"[DADOS] Alíneas em 2025: {sorted(df_2025['ALINEA'].dropna().unique())}")
    
    if not df_2024.empty:
        print(f"[DADOS] Espécies em 2024: {sorted(df_2024['ESPECIE'].dropna().unique())}")
        if 'ALINEA' in df_2024.columns:
            print(f"[DADOS] Alíneas em 2024: {sorted(df_2024['ALINEA'].dropna().unique())}")
    ...
    print(f"[DADOS] Espécies combinadas (2024+2025): {todas_especies}")
    
    for especie in todas_especies:
        print(f"\n[PROCESSANDO] Processando espécie: {especie}")
        ...
        if not nome_especie:
            print(f"   [ERRO] Nome da espécie não encontrado")
            continue
            
        print(f"   [INFO] Nome: {nome_especie}")
        ...
        print(f"   [DADOS] Registros 2025: {len(df_especie_2025)}")
        print(f"   [DADOS] Registros 2024: {len(df_especie_2024)}")
        ...
        print(f"   [VALOR] Total 2025: R$ {valor_2025_especie:,.2f}")
        print(f"   [VALOR] Total 2024: R$ {valor_2024_especie:,.2f}")
        
        if valor_2025_especie == 0 and valor_2024_especie == 0:
            print(f"   [AVISO] Valores zerados - pulando espécie")
            continue
        ...
        print(f"   [ALINEAS] Alíneas combinadas (2024+2025): {[a['ALINEA'] for a in alineas]}")
        print(f"   [DADOS] Total de alíneas: {len(alineas)}")
        ...
        for alinea in alineas:
            ...
            print(f"      [ALINEA] Processando alínea: {codigo_alinea} - {nome_alinea}")
            ...
            print(f"         [VALOR] Alínea 2025: R$ {valor_2025_alinea:,.2f}")
            print(f"         [VALOR] Alínea 2024: R$ {valor_2024_alinea:,.2f}")
            
            if valor_2025_alinea == 0 and valor_2024_alinea == 0:
                print(f"         [AVISO] Valores zerados - pulando alínea")
                continue
            ...
            print(f"         [OK] Alínea adicionada ao relatório")
    ...
    print(f"[OK] Relatório de receitas industriais gerado: {len(dados_numericos)} linhas (espécies + alíneas + total)")
    print(f"[INFO] NOUGs com saldo: {len(resumo_nougs)} unidades")
    print(f"[INFO] Comparativo mensal: {len(comparativo_mensal)} meses")
        ...
        if not df_valido.empty:
            ...
            for _, row in alineas_unicas.iterrows():
                ...
                if codigo_alinea and nome_alinea and codigo_alinea != 'nan' and nome_alinea != 'nan':
                    ...
                    print(f"   [OK] Adicionada: {codigo_alinea} - {nome_alinea}")
                else:
                    print(f"   [ERRO] Ignorada (inválida): {codigo_alinea} - {nome_alinea}")
        else:
            print("   [AVISO] Nenhum registro válido encontrado")
    else:
        print("   [ERRO] Colunas ALINEA ou NOALINEA não encontradas")
    
    print(f"   [TOTAL] Total de alíneas retornadas: {len(alineas)}")
    return alineas
...
def _gerar_resumo_nougs_com_saldo(df_2025, motor):
    ...
    if 'NOUG' not in df_2025.columns:
        print("[AVISO] Coluna 'NOUG' não encontrada")
        return []
...
def _gerar_comparativo_mensal_acumulado(df_completo, motor, mes_referencia):
    ...
    """
    try:
        # Filtra apenas origens industriais (15 e 75)
        df_industriais = df_completo[df_completo['ORIGEM'].isin(['15', '75'])].copy()
        
        if df_industriais.empty or 'INMES' not in df_industriais.columns:
            print("[AVISO] Dados insuficientes para comparativo mensal")
            return []
        
        # Obtém meses disponíveis em 2025
        meses_2025 = sorted(df_industriais[df_industriais['COEXERCICIO'] == 2025]['INMES'].dropna().unique())
        meses_2024 = sorted(df_industriais[df_industriais['COEXERCICIO'] == 2024]['INMES'].dropna().unique())
        
        if not meses_2025:
            return []
        
        max_mes = max(meses_2025)
        print(f"[INFO] Gerando comparativo mensal até mês {max_mes-1} (referência: {max_mes})")
        ...
        for mes_atual in range(1, max_mes):  # Para até max_mes-1
            ...
            df_2025_acum = df_industriais[
                (df_industriais['COEXERCICIO'] == 2025) &
                (df_industriais['INMES'] <= mes_atual)
            ]
            ...
            df_2024_acum = df_industriais[
                (df_industriais['COEXERCICIO'] == 2024) &
                (df_industriais['INMES'] <= mes_atual)
            ]
        ...
        print(f"[OK] Comparativo mensal gerado: {len(comparativo)} períodos")
        return comparativo
        
    except Exception as e:
        print(f"[ERRO] Erro ao gerar comparativo mensal: {e}")
        return []