...
# Adiciona os métodos que estavam fora da classe
def cache_pdf_sisgepat(dados_sisgepat: Dict, pdf_path: str):
    """Cache específico para dados do PDF SISGEPAT"""
    file_hash = cache_service._get_file_hash(pdf_path)
    cache_key = f"sisgepat_pdf_{file_hash}"
    
    try:
        with open(cache_service._get_cache_path(cache_key), 'wb') as f:
            pickle.dump(dados_sisgepat, f)
        print(f"💾 Dados SISGEPAT cacheados")
    except Exception as e:
        print(f"⚠️ Erro ao salvar cache SISGEPAT: {e}")

def get_cached_pdf_sisgepat(pdf_path: str) -> Optional[Dict]:
    """Recupera dados do PDF SISGEPAT do cache"""
    file_hash = cache_service._get_file_hash(pdf_path)
    cache_key = f"sisgepat_pdf_{file_hash}"
    cache_path = cache_service._get_cache_path(cache_key)
    
    if cache_service._is_cache_valid(cache_path):
        try:
            with open(cache_path, 'rb') as f:
                cached_data = pickle.load(f)
...
# Adiciona as funções ao objeto cache_service
cache_service.cache_pdf_sisgepat = cache_pdf_sisgepat
cache_service.get_cached_pdf_sisgepat = get_cached_pdf_sisgepat